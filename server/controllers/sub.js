...
exports.create = async (req, res) => {
  try {
    ...
    // 새로운 Sub 를 생성하여 res.json() 형태로 보낸다.

    const { name, parent } = req.body;
    res.json(await new Sub({ name, parent, slug: slugify(name) }).save());
  } catch (err) {
    console.log("SUB CREATE CONTROLLERS ERR ---->", err);
    res.status(400).send("Failed to create a new Sub");
  }
};
...
exports.list = async (req, res) => {
  ...
  // Sub 에 있는 모든 데이터를 array 에 묶어서 불러온다
...
exports.update = async (req, res) => {
  ...
  // 일단 findOneAndUpdate 에 첫번째 인자로 slug 로 특정 Sub 를 찾아내고,
...
exports.remove = async (req, res) => {
  try {
    const deletion = await Sub.findOneAndDelete({
      slug: req.params.slug,
    });
    res.json(deletion);
  } catch (err) {
    res.status(400).send("Failed to delete Sub");
  }
};
